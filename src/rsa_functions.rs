...
impl RsaKeys {
    ...
    pub fn read_from_files<P: AsRef<Path>>(
        ...
        let unsecure_passphrase = secure_passphrase.to_unsecure_string();
        ...
        match &self.rsa_private_key {
            Some(rsa) => {
                match base64::decode(&encrypted_data) {
                    Err(_) => {
                        return Err("Could not base64 decode given value");
                    }
                    Ok(raw_data) => {
                        let mut buf: Vec<u8> = vec![0; rsa.size() as usize];
                        match rsa.private_decrypt(&raw_data, &mut buf, Padding::PKCS1){
                            Err(_) => {
                                return Err("Could not rsa decrypt given value");
                            },
                            Ok(_) => {
                                let decrypted_data = String::from_utf8(buf)
                                .unwrap_or(String::from("could not convert decrypt data to utf8!"));
