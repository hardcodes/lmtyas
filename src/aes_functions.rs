    ...
    fn to_aes_enrypted_b64(&self) -> Result<AesEncryptionData, &'static str> {
        ...
        match encrypt(cipher, &key_buf, Some(&iv_buf), &self.as_bytes()) {
            Err(e) => {
                ...
                return Err("could not aes encrypt data!");
            }
            Ok(encrypted_data) => {
                ...
                return Ok(aes_encryption_result);
            }
        };
    }
}
...
impl DecryptAes for String{
    ...
    /// - `String` - plaintext
    fn decrypt_b64_aes(&self, key_base64: &str, iv_base64: &str) -> Result<String, Box<dyn Error>> { 
        let encrypted_data = base64::decode_config(&self.trim_matches(char::from(0)), base64::URL_SAFE)?;
        ...
        let p = (String::from_utf8(plaintext)?).to_string();
        Ok(p)
    }
}
